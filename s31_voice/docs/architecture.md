...
### 6.6 音量：一个旋钮，两个写者（2026-09-16 起修，09-17 补完）

用户报「音量调到1」被无视，并提醒："音量调低后，不要被之前那个指令结束后恢复高音量的逻辑覆盖。"
查下来是**两个独立的 bug**，而第二个比提醒的还要严重。修第二个的时候，
我又亲手引进了第三个（Bug 3），下面一并记着 —— 这一节真正的教训在那里。
...
#### Bug 3：修 A 的时候亲手引进来的 —— 音量永久停在 12

上面 A 的修法是"当前音量本来就不高于档位时，直接不参与"。第一版把这个早退
写在了 `duck()` 的**最外层**，也就是排在"重入就 cancel 掉上一个恢复任务"之后：

```python
if self._duck_task and not self._duck_task.done():
    self._duck_task.cancel()          # ① 上一次的恢复被取消
if cur <= level:
    return                            # ② 音量已经是 12 了 → 直接走人
```

于是压制窗口内的**第二次唤醒**走的是这条路：①把上一次排好的恢复任务取消掉，
②因为音量此刻正是被我们压到的 12，判定"没什么可压的"直接返回 —— 新的恢复任务
再也没排上。**系统音量就此永久停在 12**，和 §6.6 开头那个"一晚上之后什么都听不见"
的老故障一模一样，只是这回是我新写进去的。

现场表现是日志里连着刷：

```
当前音量 12 看着像是上次没恢复的压制值，按 13 记原始音量
```

——`_recall_duck_from()` 的兜底在反复救场，而它本来只该在"进程被重启过"这种
罕见情况下出现一次。**一条本该罕见的日志开始规律地刷，本身就是一条故障信号。**

修法是把早退**挪进 `else`（即"不在压制中"）分支**：既然已经在压制中，就说明
上一次确实压下去了、原始值也记着，这次只该把计时器往后推，没有"不参与"这个选项。
同时 `_ducked_to` 只在真的写下去时才更新 —— 记一个我们没写过的值，会让恢复时的
compare-and-swap 失去意义。

> 漏掉它的原因值得单记一笔：我把"压制窗口内再次唤醒"当成边缘情况。
> 可**连续对话本来就是一唤醒接一唤醒**，压制窗口 8 秒，第二次唤醒落在窗口内
> 是常态而不是例外。修 A 时我只按"用户把音量设到 1 然后说一句唤醒词"这一个
> 场景验证，那条路径根本不经过重入分支。
> 一个只测了自己想修的那条路的补丁，等于没测。

回归在 `test_control_safety.py::VolumeOwnershipTests` 和 `::VolumeIntentTests`。
其中两条是**对照**，专门防止"为了修某一条把压音量本身废掉"：
`test_untouched_duck_still_restores`（没人插手时必须照旧恢复）和
`test_repeated_wakes_still_restore`（连唤 3 次之后仍要恢复到最早那个原始音量 ——
就是 Bug 3 那条路）。
