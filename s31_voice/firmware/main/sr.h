...
/* 不经过唤醒词，直接再开一次听命令词的窗口（连续对话）。
 *
 * 只收命令词、不录兜底音频：窗口开着的时候屋里往往还有别的声音，
 * 而全量 ASR 在有噪音时基本废掉（实测服务端收到的是「🎼我唱唱给的算」）。
 * 与其录一段听不懂的音上传、再等一秒半告诉用户"这个我还不会"，
 * 不如安静地关窗 —— 用户会自然地重新说唤醒词，那条路是干净的。
 *
 * ms 是窗口时长，比首轮短：真正的追问来得很快，窗口长出来的部分几乎全是暴露面。 */
