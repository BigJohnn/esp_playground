...
_RULES: list[tuple[str, Domain, re.Pattern[str]]] = [
    ...
    # (?!灯) 是踩出来的：「切换台灯」里的"换台"会命中 `换一?个?台`，
    # 于是说切换台灯变成了给收音机换电台（2026-09-17 实测）。
    # 「台」这个字同时属于"电台"和"台灯"，只能靠后面那个字分开。
    ("fm_station_next", "tivoli", re.compile(
        r"下一?个?(?:电)?台(?!灯)|下个台(?!灯)|换一?个?台(?!灯)|切一?个?台(?!灯)"
        r"|下一个频道|搜台")),
    ...
    # 「切换歌曲」必须在这儿接住。它以前落到灯的 toggle 上（那条规则裸着一个
    # 「切换」，句子里出现就认领），于是说"切换歌曲"把灯给切了 —— 2026-09-17 实测。
    # 和「音量调到1」被 set_pct 抢走是同一类：**泛化规则没锚定，就会去抢别人的句子**。
    ("music_next", "music", re.compile(
        r"下一首|下首|换一首|换首|切歌|跳过这首"
        r"|(?:切换|换|切|下)(?:一)?(?:个|首)?(?:歌曲?|音乐)"
        r"|(?:歌曲?|音乐)(?:切换|换一个|换个)")),
    ...
    # **整句锚定**。裸的「切换」谁的句子都抢：「切换歌曲」「切换音源」里都有它。
    # 隔壁几条泛化词规则（amb_lower / amb_prev）本来就是 ^...$ 的，这条漏了。
    # 两条：带「灯」的随便怎么说，不带设备名的**必须整句就是这个词**。
    # 裸的「切换」谁的句子都抢 ——「切换歌曲」「切换音源」里都有它，
    # 实测就是这么把灯给切了（2026-09-17）。隔壁几条泛化词规则
    # （amb_lower / amb_prev）本来就是 ^...$ 的，这条一直漏着。
    ("toggle",     "light", re.compile(
        r"(?:灯|台灯|吊灯|壁灯).{0,3}?(?:切换|反过来|反一下)"
        r"|(?:切换|反过来|反一下).{0,3}?(?:灯|台灯|吊灯|壁灯)"
        r"|^(?:切换|反过来|反一下)$")),
    # ---- 只说了个设备名，没说要它干什么 ----
    #
...
# 原来这里写的是"它们本身不一定差 —— 是 23 条一起上的时候全都被稀释了"。
# **2026-09-14 证伪了这个解释**：M21 烧了一张只有两条词的表（见 README 4.1.26），
# 「声音大一点 / 声音小一点」在没有任何竞争者的情况下仍然 0/6 静默失败，
# 而同一条路径换成「打开台灯 / 关闭台灯」是 6/6。所以至少这两条是词本身不行，
# 不是被稀释。其余几条没单独验过，别拿"稀释"当理由把它们加回来。
